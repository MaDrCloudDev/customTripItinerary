<script lang="ts">
	type ButtonVariant = 'primary' | 'secondary' | 'active' | 'icon' | 'nav';
	...
	const variantClasses = {
		...
		icon: 'px-3 py-2 text-xs rounded-lg font-medium transition-all duration-300 flex flex-col items-center gap-0.5 min-w-[70px]',
	...
	const baseClass = variantClasses[variant as keyof typeof variantClasses] || variantClasses.primary;
</script>
