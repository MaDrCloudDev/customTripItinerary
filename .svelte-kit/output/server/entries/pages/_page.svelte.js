...
function CruiseHeader($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(`<header class="relative overflow-hidden bg-gradient-to-br from-rose-400 via-pink-400 to-purple-500 no-print"><div class="absolute inset-0 opacity-10"><div class="absolute top-10 left-10 text-6xl">🏺</div> <div class="absolute top-20 right-20 text-4xl">🐪</div> <div class="absolute bottom-10 left-1/4 text-5xl">🏛️</div> <div class="absolute bottom-20 right-1/3 text-3xl">🌅</div></div> <div class="relative container mx-auto px-6 py-8 text-center"><div class="mb-6"><div class="inline-flex items-center gap-4 mb-4"><div class="w-16 h-0.5 bg-gradient-to-r from-transparent to-amber-500"></div> <span class="text-4xl">🚢</span> <div class="w-16 h-0.5 bg-gradient-to-l from-transparent to-amber-500"></div></div> <h1 class="text-3xl md:text-4xl font-bold text-white drop-shadow-lg mb-3">${escape_html(cruiseInfo.title)}</h1> <p class="text-lg md:text-xl text-rose-100 font-medium mb-6">${escape_html(cruiseInfo.subtitle)}</p></div> <div class="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl mx-auto"><div class="bg-white/20 backdrop-blur-sm rounded-2xl p-4 shadow-lg border border-white/30"><div class="text-2xl mb-2">📋</div> <div class="text-sm text-rose-100 font-medium">Booking Number</div> <div class="text-lg font-bold text-white">${escape_html(cruiseInfo.bookingNumber)}</div></div> <div class="bg-white/20 backdrop-blur-sm rounded-2xl p-4 shadow-lg border border-white/30"><div class="text-2xl mb-2">👥</div> <div class="text-sm text-rose-100 font-medium">Travelers</div> <div class="text-lg font-bold text-white">${escape_html(cruiseInfo.passengers)}</div></div> <div class="bg-white/20 backdrop-blur-sm rounded-2xl p-4 shadow-lg border border-white/30"><div class="text-2xl mb-2">🏠</div> <div class="text-sm text-rose-100 font-medium">Stateroom</div> <div class="text-lg font-bold text-white">${escape_html(cruiseInfo.stateroom)}</div></div></div></div></header>`);
  });
}
...
function Navigation($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(`<nav class="bg-white/95 backdrop-blur-md border-b border-rose-200/50 py-3 no-print sticky top-0 z-50 shadow-lg"><div class="container mx-auto px-6"><div class="flex items-center justify-between gap-4"><button${attr("disabled", currentDayIndex === 0, true)} class="group px-3 py-2 rounded-full bg-gradient-to-r from-rose-500 to-pink-500 text-white font-medium disabled:from-gray-300 disabled:to-gray-400 disabled:cursor-not-allowed transition-all duration-300 hover:shadow-lg hover:scale-105 flex items-center gap-2"><span class="group-hover:-translate-x-1 transition-transform">←</span> <span>Previous</span></button> <div class="text-center flex-1 bg-rose-50/80 backdrop-blur-sm rounded-xl py-2 px-4 border border-rose-200/50"><div class="text-base font-bold bg-gradient-to-r from-rose-600 to-purple-600 bg-clip-text text-transparent">Day ${escape_html(currentDayIndex + 1)} of ${escape_html(totalDays)}</div> <div class="text-sm text-gray-700 font-medium">${escape_html(currentDay.dayName)} • ${escape_html(currentDay.location)}</div> <div class="text-xs text-gray-600">${escape_html(formatDate(currentDay.date))}</div></div> <div class="flex items-center gap-2"><button${attr_class(`px-3 py-1.5 text-xs rounded-full font-medium transition-all duration-300 ${stringify(
      "bg-gradient-to-r from-rose-500 to-pink-500 text-white shadow-md"
    )}`)}>📅 Day</button> <button${attr_class(`px-3 py-1.5 text-xs rounded-full font-medium transition-all duration-300 ${stringify("bg-rose-100 text-rose-700 hover:bg-rose-200")}`)}>🕐 Timeline</button> <button${attr_class(`px-3 py-1.5 text-xs rounded-full font-medium transition-all duration-300 ${stringify("bg-rose-100 text-rose-700 hover:bg-rose-200")}`)}>📊 Overview</button> <button class="px-3 py-1.5 text-xs rounded-full bg-gradient-to-r from-emerald-500 to-teal-500 text-white font-medium hover:shadow-md hover:scale-105 transition-all duration-300">🖨️ Print</button></div> <button${attr("disabled", currentDayIndex === totalDays - 1, true)} class="group px-3 py-2 rounded-full bg-gradient-to-r from-rose-500 to-pink-500 text-white font-medium disabled:from-gray-300 disabled:to-gray-400 disabled:cursor-not-allowed transition-all duration-300 hover:shadow-lg hover:scale-105 flex items-center gap-2"><span>Next</span> <span class="group-hover:translate-x-1 transition-transform">→</span></button></div></div></nav>`);
  });
}
const getColorClass = (slot) => {
  const colorClasses = {
    excursion: "bg-gradient-to-br from-blue-100 to-indigo-200 border-blue-300 text-blue-800 shadow-blue-200/50",
    dining: "bg-gradient-to-br from-orange-100 to-amber-200 border-orange-300 text-orange-800 shadow-orange-200/50",
    cultural: "bg-gradient-to-br from-purple-100 to-violet-200 border-purple-300 text-purple-800 shadow-purple-200/50",
    nature: "bg-gradient-to-br from-emerald-100 to-green-200 border-emerald-300 text-emerald-800 shadow-emerald-200/50",
    free: "bg-gradient-to-br from-gray-100 to-slate-200 border-gray-300 text-gray-700 shadow-gray-200/50",
    travel: "bg-gradient-to-br from-yellow-100 to-amber-200 border-yellow-300 text-yellow-800 shadow-yellow-200/50",
    meals: "bg-gradient-to-br from-rose-100 to-pink-200 border-rose-300 text-rose-800 shadow-rose-200/50"
  };
...
function ActivityCard($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    if (slot.location || slot.cost || slot.notes) {
      ...
      if (slot.location) {
        $$renderer2.push("<!--[-->");
        $$renderer2.push(`<button class="bg-white/40 backdrop-blur-sm rounded-xl p-3 border border-white/30 hover:bg-white/60 transition-colors cursor-pointer text-left w-full"><div class="text-xs font-bold opacity-80 mb-1">📍 Location</div> <div class="text-sm font-semibold flex items-center gap-1">${escape_html(slot.location)} <span class="text-xs opacity-70">🗺️</span></div></button>`);
      } else {
        $$renderer2.push("<!--[!-->");
      }
      $$renderer2.push(`<!--]--> `);
...
function SingleDayView($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(`<div class="bg-white/80 backdrop-blur-sm border border-rose-200/50 rounded-3xl p-8 mb-8 shadow-xl shadow-rose-200/40"><div class="text-center mb-8 pb-6 border-b border-rose-200/50"><div class="inline-flex items-center gap-6 mb-4"><div class="w-12 h-0.5 bg-gradient-to-r from-transparent to-rose-400"></div> <span class="text-4xl">🏺</span> <div class="text-center"><h2 class="text-3xl font-bold bg-gradient-to-r from-rose-600 to-purple-600 bg-clip-text text-transparent mb-2">Day ${escape_html(currentDay.dayNumber)}</h2> <p class="text-lg text-gray-700 font-medium">${escape_html(formatDate(currentDay.date))}</p></div> <span class="text-4xl">🏺</span> <div class="w-12 h-0.5 bg-gradient-to-l from-transparent to-rose-400"></div></div> <button class="inline-flex items-center gap-3 bg-rose-50 hover:bg-rose-100 rounded-full px-6 py-3 border border-rose-200 transition-colors cursor-pointer"><span class="text-2xl">📍</span> <span class="text-xl font-bold text-rose-800">${escape_html(currentDay.location)}</span> <span class="text-sm text-rose-600">🗺️</span></button></div> <div class="space-y-4"><!--[-->`);
...
function CruiseFooter($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    let { cruiseInfo } = $$props;
    $$renderer2.push(`<footer class="text-center py-8 bg-gradient-to-r from-rose-400 via-pink-400 to-purple-500 border-t border-rose-300/50"><div class="container mx-auto px-6"><div class="inline-flex items-center gap-4 mb-4"><div class="w-16 h-0.5 bg-gradient-to-r from-transparent to-white/60"></div> <span class="text-2xl">🏺</span> <div class="w-16 h-0.5 bg-gradient-to-l from-transparent to-white/60"></div></div> <p class="text-lg font-semibold text-white drop-shadow-sm mb-2">Viking Egypt Cruise Itinerary</p> <p class="text-rose-100 font-medium">Generated for ${escape_html(cruiseInfo.passengers)}</p> <p class="text-rose-100 text-sm">Booking #${escape_html(cruiseInfo.bookingNumber)} • Stateroom ${escape_html(cruiseInfo.stateroom)}</p> <p class="text-rose-100 text-sm mt-2">🚢 ${escape_html(formatDate(cruiseInfo.dates.start))} to ${escape_html(formatDate(cruiseInfo.dates.end))} 🚢</p></div></footer>`);
  });
}
