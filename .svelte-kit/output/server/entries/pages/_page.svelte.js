      ...
      timeSlots: [
        {
          startTime: "7:00",
          endTime: "12:00",
          activity: "Sailing & Breakfast",
          description: "Relax and enjoy ship amenities while sailing",
          type: "free",
          color: "free",
          icon: "⛵"
        },
        ...
        },
        {
          startTime: "15:30",
          endTime: "19:00",
          activity: "Return to Ship & Dinner",
    ...
    // Day 7: Nov 25 - Esna Egypt - 8:15am Village of Esna by Foot
    {
      ...
      timeSlots: [
        ...
        },
        {
          startTime: "18:00",
          endTime: "21:00",
          activity: "Evening Sailing & Dinner",
          description: "Enjoy ship amenities and dinner as we sail south",
          type: "meals",
          color: "meals",
          icon: "🍽️"
        }
    ...
    // Day 9: Nov 27 - Aswan Egypt - 8am Nubian Village, Philae Temple, and the Papyrus Institute
    {
      ...
      timeSlots: [
        ...
        },
        {
          startTime: "19:00",
          endTime: "21:00",
          activity: "Dinner with Aswan Views",
          description: "Enjoy Aswan views from ship during dinner",
          type: "meals",
          color: "meals",
          icon: "🍽️"
        }
    ...
    // Day 10: Nov 28 - Edfu Egypt - 8:30am Edfu Temple
    {
      ...
      timeSlots: [
        ...
        },
        {
          startTime: "10:30",
          endTime: "18:00",
          activity: "Sailing North on the Nile",
        ...
        },
        {
          startTime: "18:00",
          endTime: "21:00",
          activity: "Dinner & Evening on Deck",
          description: "Relaxing evening dinner on the Nile",
          type: "meals",
          color: "meals",
          icon: "🍽️"
        }
...
const getColorClass = (slot) => {
  const colorClasses = {
    excursion: "bg-gradient-to-br from-amber-100 to-amber-200 border-amber-400 text-amber-900 shadow-amber-200/50",
    ...
    travel: "bg-gradient-to-br from-sky-100 to-sky-200 border-sky-400 text-sky-900 shadow-sky-200/50",
    meals: "bg-gradient-to-br from-red-100 to-orange-200 border-red-400 text-red-900 shadow-red-200/50"
  };
...
function ActivityCard($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    const colorClass = getColorClass(slot);
    ...
    $$renderer2.push(`<!--]--> <div${attr_class(`flex items-start gap-6 ${stringify(colorClass)} rounded-2xl p-6 border shadow-lg relative z-10 hover:shadow-xl transition-all duration-300 hover:scale-[1.02]`)}><div class="text-center min-w-32 bg-white/60 backdrop-blur-sm rounded-xl p-4 border border-white/40 shadow-sm"><div class="text-lg font-bold text-gray-800 mb-2">${escape_html(formatTime(slot.startTime))}</div> <div class="text-sm font-medium text-gray-600 mb-2">↓</div> <div class="text-lg font-bold text-gray-800 mb-3">${escape_html(formatTime(slot.endTime))}</div> <div class="text-sm bg-white/70 rounded-full px-3 py-1 font-bold text-gray-700 border">${escape_html(duration)}</div></div> <div class="flex-1"><div class="flex items-start gap-4 mb-4"><span class="text-3xl drop-shadow-sm">${escape_html(slot.icon)}</span> <div class="flex-1"><h3 class="text-xl font-bold leading-tight mb-2">${escape_html(slot.activity)}</h3> <p class="text-base leading-relaxed opacity-90">${escape_html(slot.description)}</p></div></div> `);
    if (slot.location || slot.cost || slot.notes) {
      $$renderer2.push("<!--[-->");
      $$renderer2.push(`<div class="space-y-3 mt-4 pt-4 border-t border-white/40">`);
      if (slot.location) {
        $$renderer2.push("<!--[-->");
        $$renderer2.push(`<button class="bg-blue-100/60 backdrop-blur-sm rounded-xl p-4 border-2 border-blue-300 hover:bg-blue-200/80 hover:border-blue-500 transition-all duration-300 cursor-pointer text-left w-full transform hover:scale-[1.02] shadow-md hover:shadow-lg"><div class="flex items-center gap-2"><span class="text-lg">📍</span> <div class="flex-1"><div class="text-sm font-bold text-blue-800 mb-1">Location</div> <div class="text-base font-semibold text-blue-900">${escape_html(slot.location)}</div></div> <span class="text-sm font-bold bg-blue-200 px-3 py-1 rounded-full text-blue-800 flex items-center gap-1">View Map <span class="text-xs">↗</span></span></div></button>`);
      } else {
        $$renderer2.push("<!--[!-->");
      }
      $$renderer2.push(`<!--]--> `);
      if (slot.cost) {
        $$renderer2.push("<!--[-->");
        $$renderer2.push(`<div class="bg-white/50 backdrop-blur-sm rounded-xl p-4 border border-white/40"><div class="flex items-center gap-2"><span class="text-lg">💰</span> <div><div class="text-sm font-bold text-gray-700 mb-1">Cost</div> <div class="text-base font-semibold text-gray-800">${escape_html(slot.cost)}</div></div></div></div>`);
      } else {
        $$renderer2.push("<!--[!-->");
      }
      $$renderer2.push(`<!--]--> `);
      if (slot.notes) {
        $$renderer2.push("<!--[-->");
        $$renderer2.push(`<div class="bg-white/50 backdrop-blur-sm rounded-xl p-4 border border-white/40"><div class="flex items-start gap-2"><span class="text-lg">📝</span> <div><div class="text-sm font-bold text-gray-700 mb-1">Notes</div> <div class="text-base font-semibold text-gray-800 leading-relaxed">${escape_html(slot.notes)}</div></div></div></div>`);
      } else {
        $$renderer2.push("<!--[!-->");
      }
      $$renderer2.push(`<!--]--></div>`);
...
function SingleDayView($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(`<div class="bg-white/80 backdrop-blur-sm border border-blue-400/50 rounded-3xl p-8 mb-8 shadow-xl shadow-blue-400/40"><div class="text-center mb-8 pb-6 border-b border-blue-400/50"><div class="mb-4"><h2 class="text-4xl font-bold bg-gradient-to-r from-blue-600 to-emerald-600 bg-clip-text text-transparent mb-2">📋 Day ${escape_html(currentDay.dayNumber)}</h2> <p class="text-xl text-gray-700 font-medium">${escape_html(formattedDate)}</p></div> <button class="inline-flex items-center gap-3 bg-blue-100 hover:bg-blue-200 rounded-full px-8 py-4 border-2 border-blue-400 hover:border-blue-600 transition-all duration-300 cursor-pointer shadow-lg hover:shadow-xl transform hover:scale-105"><span class="text-2xl">📍</span> <span class="text-xl font-bold text-blue-900">${escape_html(currentDay.location)}</span> <span class="text-sm font-bold text-blue-700 bg-blue-200 px-2 py-1 rounded-full flex items-center gap-1">View Map <span class="text-xs">↗</span></span></button> <div class="mt-6 flex flex-wrap justify-center gap-3 text-xs"><div class="flex items-center gap-1"><div class="w-4 h-4 bg-gradient-to-br from-amber-100 to-amber-200 border border-amber-400 rounded"></div> <span>Excursions</span></div> <div class="flex items-center gap-1"><div class="w-4 h-4 bg-gradient-to-br from-purple-100 to-purple-200 border border-purple-400 rounded"></div> <span>Cultural</span></div> <div class="flex items-center gap-1"><div class="w-4 h-4 bg-gradient-to-br from-sky-100 to-sky-200 border border-sky-400 rounded"></div> <span>Travel</span></div> <div class="flex items-center gap-1"><div class="w-4 h-4 bg-gradient-to-br from-red-100 to-orange-200 border border-red-400 rounded"></div> <span>Meals</span></div> <div class="flex items-center gap-1"><div class="w-4 h-4 bg-gradient-to-br from-lime-100 to-lime-200 border border-lime-400 rounded"></div> <span>Free Time</span></div></div></div> <div class="space-y-4"><!--[-->`);
